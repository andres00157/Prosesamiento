    ...
    def find_peaks(accumulator, nhood, accumulator_threshold, N_peaks):
        ...
        while not done:
            ...
            if acc_copy[p, q] >= accumulator_threshold:
                ...
                qlim_1 = np.uint32(np.max([q - nhood_center[1], 0]))
                qlim_2 = np.uint32(np.min([q + nhood_center[1], acc_copy.shape[1] - 1]) + 1)
                plim_1 = np.uint32(np.max([p - nhood_center[0], 0]))
                plim_2 = np.uint32(np.min([p + nhood_center[0], acc_copy.shape[0] - 1]) + 1)
                
                acc_copy[plim_1:plim_2,qlim_1:qlim_2]=0
